...
- Recommended: set `WHATSAPP_WEBHOOK_SECRET` to a random string — compose wires it to both whisper-it (verify) and WAHA (`WHATSAPP_HOOK_HMAC_KEY`, sign), so forged webhooks are rejected with `401`.
...
- `POST /webhook` — WAHA's inbound `message` hook. When `WHATSAPP_WEBHOOK_SECRET` is set, verifies the `X-Webhook-Hmac` signature (HMAC-SHA512 of the raw body, constant-time) and rejects mismatches with `401` before doing anything else. Then acks `200` immediately and processes asynchronously (so long transcriptions don't trip WAHA's retry timeout); dedupes repeated message ids. `parseWahaEvent` maps the WAHA payload → `InboundMessage`; the `handler` does whitelist → first-message welcome → transcribe → reply (and the interactive/caption diarize flow).
