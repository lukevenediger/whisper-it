...
- **Speaker attribution (opt-in)** -- Two ways to label "who said what" — both are off by default, never inline with normal transcription. **On-device:** flip `WHISPER_DIARIZE=1` + supply `HF_TOKEN`, then tick the _Attribute speakers_ toggle before transcribing. Runs pyannote.audio 3.1 locally; labels segments `SPEAKER_00`/`SPEAKER_01`/… **Cloud (named):** open any transcription's _Attribute_ button to reach `/attribute.html`. Supply speaker names + roles, pick an OpenRouter model (server-configured key or BYOK from the browser), and the result is saved as a new sibling history entry — the original is never overwritten.
